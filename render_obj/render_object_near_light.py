...
def render_object(object_shape, params_rendering, params_camera, params_object_global,
    ...
    pose = camera_poses_set[0]

    area_light_textures = np.loadtxt(os.path.join(working_dir, ini['light']['texture_paths_csv']), delimiter=',',dtype=str)
    for i, texture_path in enumerate(area_light_textures):

        out_dir_v = os.path.join(os.path.abspath(
            out_dir), "view_{:03d}".format(i + 1))
        if not os.path.exists(out_dir_v):
            os.makedirs(out_dir_v)
        print("the photo is "+out_dir_v)
        R, T = pose[:, :3], pose[:, 3:]
        ...
        for j in light_names:
            ...
            bpy.data.objects[j].matrix_world = camera_pose_current # todo
            ...
            area_texture_node.image = bpy.data.images.load(texture_path) # todo
        ...
        #create_normal_map(filepath=out_dir_v, pose_id=i)
        #create_depth_map(filepath=out_dir_v, pose_id=i)
        if 'use_dof' in params_camera and str2bool(params_camera['use_dof']):
